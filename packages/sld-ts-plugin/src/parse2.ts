...
export function parseSLDTemplate(
  ...
  const components = getRegisteredComponents(ts, checker, node)

  return {
    type: ROOT_NODE,
    children: parseNodes(ast, spans, components),
...
export function getTemplateHTML(
  ...
  if (ts.isNoSubstitutionTemplateLiteral(template)) {
    ...
    template.templateSpans.forEach((span, i) => {
      ...
      templateText +=
        marker +
        i.toString().padStart(exprText.length, filler) +
        marker +
        span.literal.text;
    });
  }
...
function parseNodes(
  ...
  return nodes.flatMap((n) => parseNode(n, templateSpans, components));
}
...
function parseNode(
  ...
  const children = parseNodes(node.body ?? [], templateSpans, components);
